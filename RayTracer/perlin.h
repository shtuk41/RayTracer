...
inline float trillinear_interp(float c[2][2][2], float u, float v, float w)
{
	float accum = 0;
	for (int i = 0; i < 2; i++)
	{
...
static vec3 *perlin_generate()
{
	...
	for (int i = 0; i < 256; ++i)
	{
		p[i] = unit_vector(vec3(-1.0f + 2.0f * get_random(), -1.0f + 2.0f * get_random(), -1.0f + 2.0f * get_random()));
	}
